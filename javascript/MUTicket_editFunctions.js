 ...
 */
function muticketInitUserField(fieldName, getterName) {
    ...
    new Ajax.Autocompleter(
        ...
        Zikula.Config.baseURL + 'ajax.php?module=MUTicket&func=' + getterName,
        {
            paramName: 'fragment',
            minChars: 3,
 ...
 */
function muticketResetUploadField(fieldName) {
 ...
 */
function muticketResetDateField(fieldName) {
    if ($(fieldName) != undefined) {
        $(fieldName).value = '';
    }
    if ($(fieldName + 'cal') != undefined) {
        $(fieldName + 'cal').update(Zikula.__('No date set.', 'module_MUTicket'));
    }
}
 ...
 */
function muticketInitDateField(fieldName) {
    if ($('reset' + fieldName.capitalize() + 'Val') != undefined) {
        $('reset' + fieldName.capitalize() + 'Val').observe('click', function (evt) {
            evt.preventDefault();
            muticketResetDateField(fieldName);
        }).removeClassName('z-hide');
    }
}
 ...
 */
function muticketToggleRelatedItemForm(idPrefix) {
 ...
 */
function muticketResetRelatedItemForm(idPrefix) {
    // hide the sub form
    muticketToggleRelatedItemForm(idPrefix);
 ...
 */
function muticketCreateWindowInstance(containerElem, useIframe) {
    var newWindow;
 ...
 */
function muticketInitInlineWindow(objectType, containerID) {
    var found, newItem;
    ...
    relationHandler.each(function (relationHandler) {
        ...
        if (relationHandler.prefix === containerID) {
            ...
            relationHandler.windowInstance = muticketCreateWindowInstance($(containerID), true);
        }
    });
    ...
    if (found === false) {
        ...
        newItem.windowInstance = muticketCreateWindowInstance($(containerID), true);
 ...
 */
function muticketRemoveRelatedItem(idPrefix, removeId) {
 ...
 */
function muticketSelectRelatedItem(objectType, idPrefix, inputField, selectedListItem) {
    ...
    removeLink = Builder.node('a', {id: elemPrefix + 'Remove', href: 'javascript:muticketRemoveRelatedItem(\'' + idPrefix + '\', ' + newItemId + ');'}, 'remove');
    ...
    if (includeEditing === true) {
        ...
        $(elemPrefix + 'Edit').observe('click', function (e) {
            muticketInitInlineWindow(objectType, idPrefix + 'Reference_' + newItemId + 'Edit');
            e.stop();
        });
    }
    ...
    if (itemIds !== '') {
        if ($F(idPrefix + 'Scope') === '0') {
            ...
            itemIdsArr.each(function (existingId) {
                if (existingId) {
                    muticketRemoveRelatedItem(idPrefix, existingId);
                }
            });
            itemIds = '';
        } else {
    ...
    muticketResetRelatedItemForm(idPrefix);
}

/**
 * Initialise a relation field section with autocompletion and optional edit capabilities
 */
function muticketInitRelationItemsForm(objectType, idPrefix, includeEditing) {
    ...
    acOptions = {
        ...
        afterUpdateElement: function (inputField, selectedListItem) {
            ...
            muticketSelectRelatedItem(objectType, idPrefix, inputField, selectedListItem);
        }
    };
    relationHandler.each(function (relationHandler) {
    ...
    $(idPrefix + 'SelectorDoNew').observe('click', function(e) {
        muticketInitInlineWindow(objectType, idPrefix + 'SelectorDoNew');
        e.stop();
    });
    ...
    itemIdsArr.each(function (existingId) {
        ...
        if (existingId) {
            ...
            $(elemPrefix).observe('click', function (e) {
                muticketInitInlineWindow(objectType, elemPrefix);
                e.stop();
            });
        }
    });
 ...
 */
function muticketCloseWindowFromInside(idPrefix, itemId) {
        ...
        if (relationHandler['prefix'] === idPrefix) {
            ...
            if (itemId > 0) {
                ...
                if (relationHandler.acInstance !== null) {
                    ...
                    // show a message 
                    Zikula.UI.Alert(Zikula.__('Action has been completed.', 'module_muticket_js'), Zikula.__('Information','module_muticket_js'), {
