 ...
 * @version Generated by ModuleStudio 0.6.0 (http://modulestudio.de) at Sun Aug 18 17:24:14 CEST 2013.
 */

/**
 * Generic item list block base class.
 */
class MUTicket_Block_Base_ItemList extends Zikula_Controller_AbstractBlock
{
    /**
    ...
    {
        SecurityUtil::registerPermissionSchema('MUTicket:ItemListBlock:', 'Block title::');
    ...
    public function info()
    {
        ...
        return array('module'          => 'MUTicket',
                     'text_type'       => $this->__('MUTicket list view'),
                     'text_type_long'  => $this->__('Display list of MUTicket objects.'),
                     'allow_multiple'  => true,
                     'form_content'    => false,
                     'form_refresh'    => false,
                     'show_preview'    => true,
    ...
    public function display($blockinfo)
    {
        ...
        if (!isset($vars['catIds'])) {
            $primaryRegistry = ModUtil::apiFunc('MUTicket', 'category', 'getPrimaryProperty', array('ot' => $vars['objectType']));
        ...
        if (in_array($vars['objectType'], $this->categorisableObjectTypes)) {
            $properties = ModUtil::apiFunc('MUTicket', 'category', 'getAllProperties', array('ot' => $objectType));
        }
    
        // apply category filters
        if (in_array($objectType, $this->categorisableObjectTypes)) {
            if (is_array($vars['catIds']) && count($vars['catIds']) > 0) {
                $categoryFiltersPerRegistry = ModUtil::apiFunc('MUTicket', 'category', 'buildFilterClauses', array('ot' => $objectType, 'catids' => $vars['catIds']));
    ...
    protected function getSortParam($vars, $repository)
    {
        ...
        if ($vars['sorting'] == 'newest') {
            $idFields = ModUtil::apiFunc('MUTicket', 'selection', 'getIdFields', array('ot' => $vars['objectType']));
            if (count($idFields) == 1) {
    ...
    public function update($blockinfo)
    {
        ...
        $vars['objectType'] = $this->request->request->filter('objecttype', 'ticket', FILTER_SANITIZE_STRING);
        ...
        $vars['catIds'] = array($primaryRegistry => array());
        if (in_array($vars['objectType'], $this->categorisableObjectTypes)) {
            $vars['catIds'] = ModUtil::apiFunc('MUTicket', 'category', 'retrieveCategoriesFromRequest', array('ot' => $vars['objectType']));
        }
