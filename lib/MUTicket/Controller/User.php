...
class MUTicket_Controller_User extends MUTicket_Controller_Base_User
{
    ...
    public function view(array $args = array())
    {
        ...
        if (!in_array($uid, MUTicket_Util_Model::getExistingSupporterUids())) {
            if (!empty($where)) {
                $where .= ' AND ';
            }
            if (!empty($where)) {
    ...
    public function display(array $args = array())
    {
        ...
        // we get the user id
        $uid = UserUtil::getVar('uid');
        // we get ids of existing supporter, also these are not active
        ...
        // we check if the user has created the ticket
        if ($entity['createdUserId'] != $uid) {
            // if not we check if the user is a supporter
            if (!in_array($uid, $supporterUids)) {
                // if not we redirect by permission error
