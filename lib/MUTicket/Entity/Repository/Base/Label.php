 ...
 *
 * This is the base repository class for label entities.
 */
class MUTicket_Entity_Repository_Base_Label extends EntityRepository
{
    /**
    ...
    public function getDescriptionFieldName()
   {
    ...
    public function getAdditionalTemplateParameters($context = '', $args = array())
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if (in_array($args['action'], array('main', 'view'))) {
                ...
                $templateParameters['workflowStateItems'] = $listHelper->getEntries('label', 'workflowState');
            }
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('MUTicket_Entity_Label', 'tbl');
        $query = $qb->getQuery();
        $query->execute();
    }
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('MUTicket_Entity_Label', 'tbl')
     ...
     *
     * @return array|MUTicket_Entity_Label retrieved data array or MUTicket_Entity_Label instance
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false).
     *
     * @return ArrayCollection collection containing retrieved MUTicket_Entity_Label instances
     */
    ...
    protected function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $categoryFiltersPerRegistry = ModUtil::apiFunc('MUTicket', 'category', 'buildFilterClauses', array('ot' => 'label', 'catids' => $v));
    ...
    protected function applyDefaultFilters(QueryBuilder $qb, $parameters)
    {
        ...
        if (!in_array('workflowState', array_keys($parameters)) || empty($parameters['workflowState'])) {
            // per default we show approved labels only
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.labelColor LIKE \'%' . $fragment . '%\'';
        } else {
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.name LIKE \'%' . $fragment . '%\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numLabels';
        if ($useJoins === true) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('MUTicket_Entity_Label', 'tbl');
     ...
     * @param int    $excludeId  Id of labels to exclude (optional).
     *
