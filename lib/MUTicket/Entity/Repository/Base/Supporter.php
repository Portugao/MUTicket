 ...
 *
 * This is the base repository class for supporter entities.
 */
class MUTicket_Entity_Repository_Base_Supporter extends EntityRepository
{
    /**
     * @var string The default sorting field/expression.
     */
    protected $defaultSortingField = 'username';
    ...
    public function getDescriptionFieldName()
   {
    ...
    public function getAdditionalTemplateParameters($context = '', $args = array())
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if (in_array($args['action'], array('main', 'view'))) {
                ...
                $templateParameters['workflowStateItems'] = $listHelper->getEntries('supporter', 'workflowState');
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('MUTicket_Entity_Supporter', 'tbl');
        $query = $qb->getQuery();
        $query->execute();
    }
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('MUTicket_Entity_Supporter', 'tbl')
     ...
     *
     * @return array|MUTicket_Entity_Supporter retrieved data array or MUTicket_Entity_Supporter instance
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false).
     *
     * @return ArrayCollection collection containing retrieved MUTicket_Entity_Supporter instances
     */
    ...
    protected function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $categoryFiltersPerRegistry = ModUtil::apiFunc('MUTicket', 'category', 'buildFilterClauses', array('ot' => 'supporter', 'catids' => $v));
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        $where .= 'tbl.username LIKE \'%' . $fragment . '%\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.supportcats LIKE \'%' . $fragment . '%\'';
        } else {
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.username LIKE \'%' . $fragment . '%\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numSupporters';
        if ($useJoins === true) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('MUTicket_Entity_Supporter', 'tbl');
     ...
     * @param int    $excludeId  Id of supporters to exclude (optional).
     *
