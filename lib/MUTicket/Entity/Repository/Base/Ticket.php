 ...
 */
class MUTicket_Entity_Repository_Base_Ticket extends EntityRepository
{
    /**
    ...
    public function getDescriptionFieldName()
   {
    ...
    public function getAdditionalTemplateParameters($context = '', $args = array())
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if (in_array($args['action'], array('main', 'view'))) {
                ...
                $templateParameters['currentStateItems'] = $listHelper->getEntries('ticket', 'currentState');
                $booleanSelectorItems = array(
                    array('value' => 'no', 'text' => __('No')),
                    array('value' => 'yes', 'text' => __('Yes'))
                );
                $templateParameters['stateItems'] = $booleanSelectorItems;
                $templateParameters['ratedItems'] = $booleanSelectorItems;
            }
            ...
            $objectType = 'ticket';
            $templateParameters[$objectType . 'ThumbPresetImages'] = $imageHelper->getPreset($objectType, 'images', $context, $args);
            $templateParameters[$objectType . 'ThumbPresetFiles'] = $imageHelper->getPreset($objectType, 'files', $context, $args);
    ...
    protected function getViewQuickNavParameters($context = '', $args = array())
    {
        ...
        $parameters = array();
        $parameters['catIdList'] = ModUtil::apiFunc('MUTicket', 'category', 'retrieveCategoriesFromRequest', array('ot' => 'ticket', 'source' => 'GET', 'controllerArgs' => $this->controllerArguments));
        ...
        $parameters['currentState'] = isset($this->controllerArguments['currentState']) ? $this->controllerArguments['currentState'] : FormUtil::getPassedValue('currentState', '', 'GET');
        $parameters['owner'] = isset($this->controllerArguments['owner']) ? $this->controllerArguments['owner'] : (int) FormUtil::getPassedValue('owner', 0, 'GET');
        ...
        $parameters['state'] = isset($this->controllerArguments['state']) ? $this->controllerArguments['state'] : FormUtil::getPassedValue('state', '', 'GET');
        $parameters['rated'] = isset($this->controllerArguments['rated']) ? $this->controllerArguments['rated'] : FormUtil::getPassedValue('rated', '', 'GET');
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('MUTicket_Entity_Ticket', 'tbl');
        $query = $qb->getQuery();
        $query->execute();
    }
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('MUTicket_Entity_Ticket', 'tbl')
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false).
     *
     * @return ArrayCollection collection containing retrieved MUTicket_Entity_Ticket instances
     */
    ...
    protected function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $categoryFiltersPerRegistry = ModUtil::apiFunc('MUTicket', 'category', 'buildFilterClauses', array('ot' => 'ticket', 'catids' => $v));
            ...
            } elseif (in_array($k, array('state', 'rated'))) {
                // boolean filter
                if ($v == 'no') {
                    $qb->andWhere('tbl.' . $k . ' = 0');
                } elseif ($v == 'yes' || $v == '1') {
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.images = \'' . $fragment . '\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.files = \'' . $fragment . '\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.dueDate = \'' . $fragment . '\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.dueText LIKE \'%' . $fragment . '%\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.currentState = \'' . $fragment . '\'';
        } else {
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.title LIKE \'%' . $fragment . '%\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
    ...
    {
        $selection = ', tblParent, tblChildren, tblRating, tblLabelticket';
    ...
    protected function addJoinsToFrom(QueryBuilder $qb)
    {
        ...
        $qb->leftJoin('tbl.rating', 'tblRating');
        $qb->leftJoin('tbl.labelticket', 'tblLabelticket');
