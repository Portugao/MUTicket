 ...
 *
 * This is the base validation class for current state entities.
 */
class MUTicket_Entity_Validator_Base_CurrentState extends MUTicket_Validator
{
    /**
    ...
    public function validateAll()
    {
        ...
        if (!$this->isStringNotLongerThan('description', 1000)) {
            $errorInfo['message'] = __f('Error! Length of field value must not be higher than %2$s (%1$s).', array('description', 1000), $dom);
            return $errorInfo;
        }
        if (!$this->isStringNotLongerThan('uploadIcon', 255)) {
            $errorInfo['message'] = __f('Error! Length of field value must not be higher than %2$s (%1$s).', array('upload icon', 255), $dom);
            return $errorInfo;
        }
     ...
     *
     * This method determines if there already exist current states with the same current state.
     *
