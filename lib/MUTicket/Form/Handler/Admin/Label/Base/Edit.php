 ...
 */
class MUTicket_Form_Handler_Admin_Label_Base_Edit extends MUTicket_Form_Handler_Admin_Edit
{
    /**
     * Pre-initialise hook.
     *
    ...
    public function preInitialize()
    {
        ...
        $this->objectTypeCapital = 'Label';
        $this->objectTypeLower = 'label';
    ...
    public function initialize(Zikula_Form_View $view)
    {
        ...
        $this->relationPresets['ticketlabel'] = FormUtil::getPassedValue('ticketlabel', '', 'GET');
        if (!empty($this->relationPresets['ticketlabel'])) {
            $relObj = ModUtil::apiFunc($this->name, 'selection', 'getEntity', array('ot' => 'ticket', 'id' => $this->relationPresets['ticketlabel']));
            if ($relObj != null) {
                $entity->addTicketlabel($relObj);
            }
        }
        ...
        if (count($this->listFields) > 0) {
            $helper = new MUTicket_Util_ListEntries($this->view->getServiceManager());
    ...
    {
        // redirect to the list of labels
        $viewArgs = array('ot' => $this->objectType);
        $url = ModUtil::url($this->name, 'admin', 'view', $viewArgs);
    
        if ($args['commandName'] != 'delete' && !($this->mode == 'create' && $args['commandName'] == 'cancel')) {
            // redirect to the detail page of treated label
            $url = ModUtil::url($this->name, 'admin', 'display', array('ot' => 'label', 'id' => $this->idValues['id']));
        }
    ...
    protected function getDefaultMessage($args, $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'submit':
                        if ($this->mode == 'create') {
                            $message = $this->__('Done! Label created.');
                        } else {
                            $message = $this->__('Done! Label updated.');
                        }
                        break;
            case 'delete':
                        $message = $this->__('Done! Label deleted.');
                        break;
            default:
                        $message = $this->__('Done! Label updated.');
                        break;
