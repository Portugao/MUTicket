...
function muticketInitRelationItemsForm(objectType, idPrefix, includeEditing)
{
    ...
    if ($(idPrefix + 'AddLink') != undefined) {
        $(idPrefix + 'AddLink').observe('click', function(e) { muticketToggleRelatedItemForm(idPrefix); });
    }
    // add handling for the cancel button
    if ($(idPrefix + 'SelectorDoCancel') != undefined) {
        $(idPrefix + 'SelectorDoCancel').observe('click', function(e) { muticketResetRelatedItemForm(idPrefix); });
    }
    ...
    relationHandler.each(function(relationHandler) {
        if (relationHandler['prefix'] == (idPrefix + 'SelectorDoNew') && relationHandler['acInstance'] == null) {
            relationHandler['acInstance'] = new Ajax.Autocompleter(
                ...
                Zikula.Config['baseURL'] + 'ajax.php?module=MUTicket&func=getItemList',
                acOptions
            );
        }
    });
