 ...
 */
class MUTicket_Base_UploadHandler
{
	/**
	...
	{
		$this->allowedObjectTypes = array('ticket');
		...
		$filesize = ModUtil::getVar('MUTicket', 'fileSize');
		$this->allowedFileSizes = array('ticket' => array('images' => $filesize));
	}
	...
	public function performFileUpload($objectType, $fileData, $fieldName)
	{
		...
		// retrieve the final file name
		$basePath = MUTicket_Util_Controller::getFileBaseFolder($objectType, $fieldName);
	...
	protected function validateFileUpload($objectType, $file, $fieldName)
	{
		...
		if ($maxSize > 0) {
			...
			if ($fileSize > $maxSize) {
				...
				return LogUtil::registerError(__f('Error! Your file is too big. Please keep it smaller than %s megabytes.', array($maxSizeKB), $dom));
			}
			...
			case 'ticket':
				switch ($fieldName) {
					case 'images':
						...
						break;
					case 'files':
						$allowedExtensions = array('pdf', 'doc', 'odt');
						break;
				}
				break;
		}
