...
// DEBUG: permission check aspect ends
    ...
    /**
     * This method provides a generic open or closed item  list overview.
     *
     * @param string  $ot           Treated object type.
     * @param string  $sort         Sorting field.
     * @param string  $sortdir      Sorting direction.
     ...
     * @param string  $tpl          Name of alternative template (for alternative display options, feeds and xml output)
     * @param boolean $raw          Optional way to display a template instead of fetching it (needed for standalone output)
     * @param int     $state        Open or closed ticket - 1 is open, 0 is closed
     * @return mixed Output.
     */
    public function viewopenclosed($args)
    {
// DEBUG: permission check aspect starts
        $this->throwForbiddenUnless(SecurityUtil::checkPermission('MUTicket::', '::', ACCESS_READ));
// DEBUG: permission check aspect ends
        ...
        $state = (int) $this->request->getGet()->filter('state', 0, FILTER_VALIDATE_INT);
        ...
        if($entityClass == 'MUTicket_Entity_Ticket') {
        	...
        	if($state == 1) {
        		$where .= " AND tbl.state = 1";
        	}
        	if($state == 0) {
        		$where .= " AND tbl.state = 0"; 
        	}
