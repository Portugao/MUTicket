...
class MUTicket_Entity_Repository_Base_Ticket extends EntityRepository
{
    ...
    public function truncateTable()
    {
        $query = $this->getEntityManager()
                 ->createQuery('DELETE MUTicket_Entity_Ticket');
        $query->execute();
    }
    ...
    public function selectSearch($fragment = '', $exclude = array(), $orderBy = '', $currentPage = 1, $resultsPerPage = 25, $useJoins = true)
    {
        ...
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.images = \'' . $fragment . '\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.files = \'' . $fragment . '\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.state LIKE \'%' . $fragment . '%\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.t_rating = \'' . $fragment . '\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.rated = \'' . $fragment . '\'';
...
// TODO - see http://code.zikula.org/core/ticket/3138
    ...
    {
        $selection = ', tblParent, tblRating, tblChildren';
        return $selection;
    }
