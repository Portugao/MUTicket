...
class MUTicket_Entity_Validator_Base_Rating extends MUTicket_Validator
{
    ...
    public function validateAll()
    {
        ...
        if (!$this->isValidNumber('ratingvalue')) {
            $errorInfo['message'] = __f('Error! Field value must be numeric (%s).', array('ratingvalue'), $dom);
            return $errorInfo;
        }
        ...
        if (!$this->isNumberNotLongerThan('ratingvalue', 12)) {
            $errorInfo['message'] = __f('Error! Length of field value must not be higher than %2$s (%1$s).', array('ratingvalue', 12), $dom);
            return $errorInfo;
        }
        return true;
    }
