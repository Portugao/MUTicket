 ...
 */
class MUTicket_Form_Handler_Base_Admin_Rating_Edit extends Zikula_Form_AbstractHandler
{
    /**
    ...
    /**
     * @var MUTicket_Entity_Rating Reference to treated rating instance.
     */
    protected $ratingRef = false;

    /**
     * @var array List of identifiers of treated rating.
     */
    ...
    public function initialize(Zikula_Form_View $view)
    {
        ...
        if ($this->mode == 'edit') {
            if (!SecurityUtil::checkPermission('MUTicket:Rating:', '::', ACCESS_EDIT)) {
            ...
            // try to guarantee that only one person at a time can be editing this rating
            $itemID = '';
            ...
            $returnUrl = ModUtil::url('MUTicket', 'admin', 'display', array('ot' => 'rating', 'id' => $this->idValues['id']));
            ModUtil::apiFunc('PageLock', 'user', 'pageLock',
                array('lockName'  => 'MUTicketRating' . $itemID,
                'returnUrl' => $returnUrl));
        } else {
            if (!SecurityUtil::checkPermission('MUTicket:Rating:', '::', ACCESS_ADD)) {
                return $this->view->setErrorMsg(LogUtil::registerPermissionError());
            }
    ...
    protected function getRedirectCodes()
    {
        ...
        $codes[] = 'adminView';
        // admin display page of treated rating
        $codes[] = 'adminDisplay';
        // main page of user area
        $codes[] = 'user';
        // user list of ratings
        $codes[] = 'userView';
        // user display page of treated rating
        $codes[] = 'userDisplay';
    ...
    protected function getDefaultReturnUrl($args, $obj)
    {
        ...
        if ($args['commandName'] != 'delete') {
            ...
            $url = ModUtil::url('MUTicket', 'admin', 'display', array('ot' => 'rating', 'id' => $this->idValues['id']));
        }
        return $url;
    }
    ...
    {
        if ($args['commandName'] == 'delete') {
            if (!SecurityUtil::checkPermission('MUTicket:Rating:', '::', ACCESS_DELETE)) {
                return $this->view->setErrorMsg(LogUtil::registerPermissionError());
            }
        }
        ...
        $ratingData = $formData['rating'];
        unset($formData['rating']);

        $repeatCreateAction = false;
        if ($args['commandName'] == 'create' && isset($ratingData['repeatcreation'])) {
            $repeatCreateAction = $ratingData['repeatcreation'];
            unset($ratingData['repeatcreation']);
        }
        ...
        if (in_array($args['commandName'], array('create', 'update'))) {
            ...
            $hook = new Zikula_ValidationHook('muticket.ui_hooks.ratings.validate_edit', new Zikula_Hook_ValidationProviders());
            $validators = $this->notifyHooks($hook)->getValidators();
            // TODO validation incomplete (#36)

            // save rating
            $successMessage = '';
            $this->updateRelationLinks($rating);
            ...
            if ($args['commandName'] == 'create') {
                if (!empty($this->ticket)) {
                    $relObjRepository = $this->entityManager->getRepository('MUTicket_Entity_Ticket');
                    $relObj = $relObjRepository->selectById($this->ticket);
                    $relObj->addRating($rating);
                }
                $this->entityManager->flush();
            }

            if ($args['commandName'] == 'create') {
                $successMessage = $this->__('Done! Rating created.');
            } else if ($args['commandName'] == 'update') {
                $successMessage = $this->__('Done! Rating updated.');
            }

            if ($args['commandName'] == 'create') {
                // store new identifier
                $this->idValues['id'] = $rating->getId();
            ...
            $hook = new Zikula_ProcessHook('muticket.ui_hooks.ratings.process_edit', $this->id, $url);
            $this->notifyHooks($hook);
        } else if ($args['commandName'] == 'delete') {
            // event handling if user clicks on delete

            // Let any hooks perform additional validation actions
            $hook = new Zikula_ValidationHook('muticket.ui_hooks.ratings.validate_delete', new Zikula_Hook_ValidationProviders());
            $validators = $this->notifyHooks($hook)->getValidators();
            // TODO validation incomplete (#36)
            ...
            LogUtil::registerStatus($this->__('Done! Rating deleted.'));

            // Let any hooks know that we have deleted an item
            $hook = new Zikula_ProcessHook('muticket.ui_hooks.ratings.process_delete', $this->id);
            $this->notifyHooks($hook);
        ...
        if ($this->mode == 'edit') {
            ModUtil::apiFunc('PageLock', 'user', 'releaseLock',
                array('lockName' => 'MUTicketRating' . $this->id));
        }
        return $this->view->redirect($this->getRedirectUrl($args, $rating, $repeatCreateAction));
    ...
    protected function getRedirectUrl($args, $obj, $repeatCreateAction = false)
    {
        ...
        switch ($this->returnTo) {
            ...
            case 'adminDisplay':
                if ($args['commandName'] != 'delete') {
                    return ModUtil::url('MUTicket', 'admin', 'display', array('ot' => 'rating', 'id' => $this->idValues['id']));
                }
                return $this->getDefaultReturnUrl($args, $obj);
            case 'user':
            ...
            case 'userDisplay':
                if ($args['commandName'] != 'delete') {
                    return ModUtil::url('MUTicket', 'user', 'display', array('ot' => 'rating', 'id' => $this->idValues['id']));
                }
                return $this->getDefaultReturnUrl($args, $obj);
            case 'account':
            ...
            case 'adminDisplayTicket':
                if (!empty($this->ticket)) {
                    return ModUtil::url('MUTicket', 'admin', 'display', array('ot' => 'ticket', 'id' => $this->ticket));
                }
                return $this->getDefaultReturnUrl($args, $obj);
            case 'userViewTicket':
                return ModUtil::url('MUTicket', 'user', 'view',
            ...
            case 'userDisplayTicket':
                if (!empty($this->ticket)) {
                    return ModUtil::url('MUTicket', 'user', 'display', array('ot' => 'ticket', 'id' => $this->ticket));
                }
                return $this->getDefaultReturnUrl($args, $obj);
            default:
                return $this->getDefaultReturnUrl($args, $obj);
