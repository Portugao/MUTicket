{* purpose of this template: inclusion template for managing related current state in admin area *}
...
{/if}
    ...
    {elseif $displayMode eq 'autocomplete'}
        ...
        {if $allowEditing eq true}
            {modurl modname='MUTicket' type='admin' func='edit' ot='currentState' assign='createLink'}
        {/if}
        ...
        <div class="muticketRelationLeftSide">
            {if isset($linkingItem.$alias)}
                {include file='admin/currentState/include_selectItemListOne.tpl'  item=$linkingItem.$alias}
            {else}
                {include file='admin/currentState/include_selectItemListOne.tpl' }
            {/if}
        </div>
        <br class="z-clearer" />
    {/if}
