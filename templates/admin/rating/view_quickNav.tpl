{* purpose of this template: ratings view filter form in admin area *}
{checkpermissionblock component='MUTicket:Rating:' instance='::' level='ACCESS_EDIT'}
{assign var='objectType' value='rating'}
<form action="{$modvars.ZConfig.entrypoint|default:'index.php'}" method="get" id="muticketRatingQuickNavForm" class="muticketQuickNavForm">
    <fieldset>
        ...
        {if !isset($ticketFilter) || $ticketFilter eq true}
            <label for="ticket">{gt text='Tickets'}</label>
            ...
            {foreach item='option' from=$listEntries}
                ...
                <option value="{$entryId}"{if $entryId eq $ticket} selected="selected"{/if}>{$option.title}</option>
            {/foreach}
            </select>
        {/if}
...
/* <![CDATA[ */
    document.observe('dom:loaded', function() {
        muticketInitQuickNavigation('rating', 'admin');
