{* purpose of this template: inclusion template for managing related ticket in admin area *}
...
{/if}
    ...
    {elseif $displayMode eq 'autocomplete'}
        ...
        {if $allowEditing eq true}
            {modurl modname='MUTicket' type='admin' func='edit' ot='ticket' assign='createLink'}
        {/if}
        ...
        <div class="muticketRelationLeftSide">
            {if isset($linkingItem.$alias)}
                {include file='admin/ticket/include_selectEditItemListOne.tpl'  item=$linkingItem.$alias}
            {else}
                {include file='admin/ticket/include_selectEditItemListOne.tpl' }
            {/if}
        </div>
        <br class="z-clearer" />
    {/if}
