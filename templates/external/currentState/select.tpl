...
<p>
    <label for="{$baseID}_id"{$leftSide}>{gt text='Current state'}:</label>
    <select id="{$baseID}_id" name="id"{$rightSide}>
        {foreach item='currentState' from=$items}{strip}
            <option value="{$currentState.id}"{if $selectedId eq $currentState.id} selected="selected"{/if}>
                {$currentState.title}
            </option>{/strip}
        {foreachelse}
