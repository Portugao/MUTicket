...
<p>
    <label for="{$baseID}_id"{$leftSide}>{gt text='Supporter'}:</label>
    <select id="{$baseID}_id" name="id"{$rightSide}>
        {foreach item='supporter' from=$items}{strip}
            <option value="{$supporter.id}"{if $selectedId eq $supporter.id} selected="selected"{/if}>
                {$supporter.username}
            </option>{/strip}
        {foreachelse}
