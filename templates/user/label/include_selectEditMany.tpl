{* purpose of this template: inclusion template for managing related labels in user area *}
...
{/if}
    ...
    {elseif $displayMode eq 'autocomplete'}
        ...
        {if $allowEditing eq true}
            {modurl modname='MUTicket' type='user' func='edit' ot='label' forcelongurl=true assign='createLink'}
        {/if}
        ...
        <div class="muticketRelationLeftSide">
            {if isset($linkingItem.$alias)}
                {include file='user/label/include_selectEditItemListMany.tpl'  items=$linkingItem.$alias}
            {else}
                {include file='user/label/include_selectEditItemListMany.tpl' }
            {/if}
        </div>
        <br class="z-clearer" />
    {/if}
